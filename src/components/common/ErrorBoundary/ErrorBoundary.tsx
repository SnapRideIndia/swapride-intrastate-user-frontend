        ...
        <SafeAreaView edges={['top', 'bottom']} style={ErrorBoundaryStyles.container}>
          <ScrollView contentContainerStyle={ErrorBoundaryStyles.scrollContainer} showsVerticalScrollIndicator={false}>
            <View style={ErrorBoundaryStyles.errorContainer}>
              ...
              <Text style={ErrorBoundaryStyles.subtitle}>We're sorry, but something unexpected happened. Please try again.</Text>

              {__DEV__ && this.state.error && (
                <View style={ErrorBoundaryStyles.errorDetails}>
                  <Text style={ErrorBoundaryStyles.errorDetailsTitle}>Error Details:</Text>
                  <Text style={ErrorBoundaryStyles.errorMessage}>{this.state.error.toString()}</Text>

                  {this.state.errorInfo && <Text style={ErrorBoundaryStyles.errorStack}>{this.state.errorInfo.componentStack}</Text>}
                </View>
              )}

              <TouchableOpacity style={ErrorBoundaryStyles.retryButton} onPress={this.resetErrorBoundary} activeOpacity={0.8}>
