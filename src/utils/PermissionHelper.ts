...
const handleBlocked = (type: PermissionType) => {
  Alert.alert('Permission Required', `Please enable ${type} permission from settings.`, [
...
export const requestMultiplePermissions = async (types: PermissionType[]): Promise<PermissionResult> => {
  ...
  for (const type of types) {
    const perms = PermissionMap[type] || [];
    const isGranted = perms.every(p => statuses[p] === RESULTS.GRANTED || reqResults[p] === RESULTS.GRANTED);
    const isBlocked = perms.some(p => statuses[p] === RESULTS.BLOCKED || reqResults[p] === RESULTS.BLOCKED);
